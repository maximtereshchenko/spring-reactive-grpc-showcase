...
openssl req -new -key server-private-key.key -passin pass:password -subj "/CN=*.us-east-1.elb.amazonaws.com/" -addext "subjectAltName=DNS:localhost,DNS:users,DNS:orders-read-side,DNS:orders-write-side" -out server-certificate-signing-request.csr
