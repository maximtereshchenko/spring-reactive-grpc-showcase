...
openssl req -new -key server-private-key.key -passin pass:password -subj "/CN=spring-reactive-grpc-showcase/" -addext "subjectAltName=DNS:*.amazonaws.com,DNS:localhost,DNS:users,DNS:orders-read-side,DNS:orders-write-side" -out server-certificate-signing-request.csr
openssl x509 -req -in server-certificate-signing-request.csr -CA ca-self-signed-certificate.pem -CAkey ca-private-key.key -passin pass:password -CAcreateserial -days 365 -out server-certificate.pem -extfile <(echo "subjectAltName=DNS:*.amazonaws.com,DNS:localhost,DNS:users,DNS:orders-read-side,DNS:orders-write-side")
